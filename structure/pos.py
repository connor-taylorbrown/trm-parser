...
class PartOfSpeechAnnotator(Annotator):
    @abstractmethod
...
class PartOfSpeechAnnotatorFactory(AnnotatorFactory):
    def create(self, node: SyntaxNode, logger: Logger):
        if not node:
            return None
        
        if isinstance(node, NonTerminal):
            return NonTerminalAnnotator(node, self.create(node.left, logger), self.create(node.right, logger), logger)
...
class TerminalAnnotator(PartOfSpeechAnnotator):
...
class NonTerminalAnnotator(PartOfSpeechAnnotator):
    def __init__(self, node: NonTerminal, left: PartOfSpeechAnnotator, right: PartOfSpeechAnnotator, logger: Logger):
        self.node = node
        self.left = left
        self.right = right
        self.logger = logger
